"""
Token probability features with Qwen3.5-4B as observer model.
...
if __name__ == "__main__":
    ...
    # ── Summary ──
    print("\n" + "="*60)
    print(f"SUMMARY: {MODEL_NAME} vs Mistral-7B-Instruct")
    print("="*60)
    mistral = {'HC3': 0.9998, 'SemEval': 0.9784, 'TuringBench': 0.4853, 'Pile': 0.9918}
    print(f"  {'Dataset':<15s} {'Mistral-7B':>10s} {'Qwen3.5-4B':>10s} {'Diff':>8s}")
    for ds in ['HC3', 'SemEval', 'TuringBench', 'Pile']:
        diff = results[ds] - mistral[ds]
        marker = "+" if diff > 0 else ""
        print(f"  {ds:<15s} {mistral[ds]:>10.4f} {results[ds]:>10.4f} {marker}{diff:>7.4f}")
    ...
    q_vals = [results[d] for d in datasets]

    bars1 = ax.bar(x - width/2, m_vals, width, label='Mistral-7B-Instruct', color='#1f77b4', alpha=0.85)
    bars2 = ax.bar(x + width/2, q_vals, width, label='Qwen3.5-4B', color='#ff7f0e', alpha=0.85)
    ...
    ax.set_ylabel('AUC')
    ax.set_title('Token Probability Features: Mistral-7B vs Qwen3.5-4B as Observer')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    fname = FIG_DIR / 'token_observer_comparison.png'
