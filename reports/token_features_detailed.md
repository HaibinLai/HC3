...
为验证 token 概率框架是否依赖特定观察者模型，我们用 3 个不同架构/规模的模型做了完整对比：

| 数据集 | Mistral-7B (7B Dense) | Qwen3.5-4B (4B Dense) | Qwen3.6-35B-A3B (3B active MoE) |
|--------|:-:|:-:|:-:|
...
![Observer Comparison](../figures/token_observer_comparison_3models.png)

**结论：**
- **Dense 4B 是最佳观察者。** 检测性能在所有数据集上最优或并列最优，GPU 显存仅 7.8 GB。
- **MoE 架构不适合做"显微镜"。** Qwen3.6-35B-A3B 虽然总参数 35B，但在 SemEval 上明显弱于 4B Dense 模型（0.9541 vs 0.9844）。MoE 的稀疏专家路由使概率分布更嘈杂，降低了 token 特征的区分度。
- **模型大小不是关键，架构才是。** Dense 架构产生更稳定、一致的概率分布，更适合提取可判别的统计特征。
- **TuringBench 仍然失败。** 三个观察者都无法检测旧模型文本，确认这是可预测性假说的固有局限。
